...
const requestConfig = {
  ...
  source: fs.readFileSync("./NFTTokenMetadata-api.js").toString(),
  //source: fs.readFileSync('./API-request-example.js').toString(),
  // secrets can be accessed within the source code with `secrets.varName` (ie: secrets.apiKey). The secrets object can only contain string values.
  secrets: { covalentApiKey: process.env.COVALENT_API_KEY ?? '' , alchemyApiKey: process.env.ALCHEMY_API_KEY ?? '', infuraAPiKey: process.env.INFURA_API_KEY ?? '', infuraAPiKeySecret: process.env.INFURA_API_KEY_SECRET ?? '',
  // ETH wallet key used to sign secrets so they cannot be accessed by a 3rd party
  walletPrivateKey: process.env["PRIVATE_KEY"],
  // args (string only array) can be accessed within the source code with `args[index]` (ie: args[0]).
  args: ["shiba inu on a rocket to the moon"],
